...
- Replaced the artboard's `transform: scale()` with `zoom` after measuring, in Chromium
  151, Firefox 153 and WebKit 26.5, that a transformed ancestor stops `position: fixed`
  and `position: sticky` from resolving against the viewport, while `zoom` produces an
  identical layout and leaves both intact. This is the decision that made DA-MEDIA-01
  implementable without JavaScript. `animation-timeline: scroll()` was rejected as the
  cross-engine mechanism because Firefox 153 still does not support it.
- Studied the behavioural reference named by DA-MEDIA-01 (`apple-q.jp`) before rebuilding
  the pin, and adopted its architecture rather than its assets: one viewport-fixed layer
  behind scrolling content, with no scroll listener participating in the pin. Recorded the
  measured evidence in `docs/scroll-motion-spec.md`.
- Split each approved contour export into one file per contour path so the contour system
  can express a depth field instead of a single lag. Verified the split layers recompose
  to the source drawing with zero differing pixels before shipping them, so the at-rest
  composition and the existing visual baselines stay valid.
- Reversed the sign of the decorative object drift: the contour planes lag the document
  and the foreground objects lead it, so the page reads as three planes. The previous
  single-direction model made every decorative element behave like background.
- Accepted a full traverse of each chapter photograph inside its pin. The approved frames
  are authored at full chapter height, and a viewport-sized crop discarded 59 % of every
  one of them. Reveal strength is a single documented constant, so the owner can dial it
  back toward a pure single-crop pin without touching the architecture.
- Measured the source document's guide layer directly (content streams, not a render) to
  recover the two first-view frames the pink annotation draws: PC `1200 × 800` and SP
  `375 × 667`, with the scroll block authored at the same `64.2 px` clearance from each
  frame's bottom edge. The guides had been catalogued as "identity only"; they are the
  only statement in the source about what the *window* is, and without them the artboard's
  width-driven scale silently pushed the cue past the fold on every window that is not the
  authored ratio.
- Chose `min(authored, fold − clearance)` over "always follow the fold" for the first-view
  cue. Following the fold would have moved the cue on every window; the min keeps the
  approved reference frame exactly wherever it fits and only intervenes on the windows
  that genuinely cannot show it. Fidelity is preserved by default and corrected by
  exception, rather than the other way round.
- Gave the SP band one artboard scale instead of two. It had been resolving decorative
  artwork in `vw` while pinning type to authored 375 px offsets, so the two families
  diverged 4 % at 390 px and 15 % at 430 px — the direct cause of the mobile prose
  leaving the line it is drawn against, and of a canvas-centred element reading as
  off-centre. The band now zooms a 375 px canvas exactly as the desktop band zooms 1200,
  and every `vw` value was restated as the canvas pixel it evaluated to at 375 so the
  approved rendering is preserved rather than re-derived.
- Replaced the scroll-motion drive model. Cap-clamped `scroll × depth` made every band
  freeze at its cap for most of the document — measured, the footer stack held one value
  across 3 000 px of scroll and the hero stack across ~13 000 px — which is why three
  rounds of raising the numbers never made the parallax read. Each layer now interpolates
  across its own crossing of the window, so it cannot saturate while it is visible and is
  still by construction once it is not. A cap is a cliff; a crossing is a curve.
- Gave decorative objects separate rise and fall limits. A single symmetric cap has to be
  small enough for the tightest neighbour, which on the product row is the 33 px gap to
  the caption; separating the two lets a drawing use the whole 245 px row above it without
  ever overrunning its own caption.
- Solved the fixed control's legibility with the page's own material rather than with a
  colour rule. Neither ink nor paper survives all four chapter frames — ROOT measures
  about 1.05 : 1 against ink and ALPINE's corner is lighter than the paper — so the
  control sits on a plate of the page surface whenever the surface behind it stops being
  paper. Over paper the plate *is* the paper, so the approved frame is untouched, and the
  contrast is a fixed 14.6 : 1 everywhere else. Focusing the control widens the plate past
  the focus ring so the indicator is never drawn onto photography.
- Recorded, without acting on it, that the contour system ships at roughly 4× the master's
  0.25 px hairline. The measurement that set it (VF-28) used percentiles contaminated by
  other ink in the same bands; integrated ink width shows the master draws contours and
  product outlines at the same weight. The product drawings were raised to match the
  contours rather than the contours lowered to match the master, because the owner has
  rejected the faithful hairline twice — but the deviation and a measured alternative
  (one whole device pixel at ~0.5 stroke-opacity: crispness instead of weight) are on
  record for them to decide.
- Rejected the paper plate behind the fixed menu control on owner review and solved the
  same problem with the page's own type treatment instead. Sampling the surface behind
  the control proved that "just change the colour" cannot mean *one* colour: the backdrop
  runs from 0.799 (paper) to 0.007 (ROOT forest) to 0.652 (ALPINE snow) and crosses the
  ink/paper break inside a single chapter, so even a per-chapter colour fails. The control
  now does what the chapter titles already do over the same photographs — a treatment the
  owner has approved four times over — and the background stays transparent everywhere.
- Kept the ROOT upper foreground as the one element with no scroll offset, and turned that
  into a second job: it is the seam between the paper page and the first photograph, so it
  is also the exact signal for when the fixed control stops being over paper. The ROOT
  chapter box starts about 260 px above the photograph a reader actually sees.
- Learned that a parallax can be present and still invisible. Two of the four hero contour
  lines run *down* the page, and a vertical translation of a vertical line slides it along
  its own direction. Travel is now perpendicular to each line's measured orientation, which
  is why the same amplitude suddenly reads. Amplitude was never the problem in any of the
  three rounds of this complaint — first saturation, then direction.
- Stopped trusting Playwright's bare `--update-snapshots`. Two footer baselines survived a
