...
| VF-43 | four photo chapters, iOS 26 Safari, bottom browser bar | **Owner rejection of VF-42.** The bands are still there over every photograph — the right colour now, but flat | VF-42 solved the wrong half. Measured on four device screenshots: the band is **58.0 CSS px** on all four chapters and its per-row luma standard deviation is **0.1**, against 11.3–15.1 in the photograph immediately above it, with the transition completing inside a single raster row. So the toolbar is not blurring the page enough to hide a photograph — solving for colour cannot solve this, because there is no photograph there to see. Solving the composite as `alpha x content + base` from two chapters gives `alpha ~ 0.72` over a dark base, uniform down the strip; under that model the shipped fill's implied content matches the frame's own lower edge on ROOT and ALPINE and misses it by about 50 luma levels on DUSK and DAWN, because a frame-wide row mean is not what the reader is looking at. A `position: fixed` box is clipped to the window and can never reach those strips, so the photograph has to be there twice: once on the pinned plate, and once as ordinary document content held still by `animation-timeline: scroll(root block)`. Device evidence for the mechanism, from the `/edge-probe` diagnostic on an iPhone at 402 x 714 against a screen-fixed reference in the same frame: **0.05 to 0.64 CSS px** through inertial scrolling at roughly 9 000 px/s, where the same page driven from a frame loop measured 69 to 225 px. Three defects were found and fixed while building it: a counter-translated layer extends the document in Chromium unless the band carries `overflow: clip`; endpoints derived from cancelling `vh` expressions leave 0.22 px behind; and endpoints left stale across an address-bar collapse put an error of 0.75 % of the scroll offset into the pin. The cost is recorded in `docs/scroll-motion-spec.md` §4.2: on the compact composition the frame's travel drops from 213.7 to 53.7 px and it starts about a status bar above the window, because the approved derivative is 927.7 px tall against an 874 px screen and cannot do both | **IMPLEMENTED CANDIDATE — TOP EDGE UNVERIFIED, DEVICE CONFIRMATION OF ALL FOUR PHOTOGRAPHS OUTSTANDING** |

| VF-44 | four photo chapters, iOS 26 Safari, both browser bars | **Owner review of VF-43 on the device.** The photograph does now reach into the bars, but "there is still something wrong — reverse scroll and both address-bar states are especially broken" | Two defects, both reproduced under control and both invisible to every test that existed. **(a) The photograph's travel never reached the mirror.** `--chapter-pan-y` was written onto the plate's `<img>`, and custom properties inherit down, not sideways: the mirror's `<img>` is a sibling subtree, so it fell back to `0%` and never moved. Measured in WebKit at 402 x 714, the photograph stepped **25 to 55 CSS px at the window edge**, by an amount that changed with the scroll offset — which is why it read worst wherever the picture was moving. The property now belongs to `.chapter-photo`, the one ancestor both images share, and the plate's own shadowing declaration is removed. Measured after: seam mismatch **0.00 px** in WebKit and Chromium across forward scroll, reverse scroll, momentum-sized jumps and window heights of 660 / 714 / 754. **(b) A mirror was revealed before its animation had applied.** Writing the keyframe endpoints is not the same as the compositor having used them: measured in WebKit, on the first frames after load the mirrors sat up to **2 681 CSS px** from their pinned position, which on a device is a flash of the wrong part of the photograph inside both bars. Chromium applied them immediately. The reveal is now gated on the layer's measured position rather than on the geometry having been written, checked for a bounded number of frames after each measure and never on the scroll path. Gating on `display` deadlocked — a `display: none` box has no box to measure — so the gate is `visibility` | **CORRECTED — MEASURED IN TWO ENGINES; DEVICE CONFIRMATION OF ALL FOUR PHOTOGRAPHS OUTSTANDING** |
| VF-45 | four photo chapters, iOS 26 Safari, status-bar strip | An independent review of the same device evidence found frames where a flat band was still present at a window edge, which the two defects above do not explain | **The photograph did not reach the top of the screen.** How far above the window it has to start was derived from `env(safe-area-inset-top)`, and WebKit 301994 has that returning 0 on several shipping iOS versions — a zero there puts the photograph's own top edge inside the status bar with the flat fill above it. Measured across 18 scroll and toolbar combinations before the change, the coverage was short by **7.4 to 36.9 CSS px**, worst at each chapter's entry, and one recorded frame carried **66.7 CSS px** of perfectly flat fill at the bottom of the screen. The estimate now takes whichever is larger of the safe-area inset and a documented floor, so it survives the inset reporting zero. The two strips are also not a fixed share of the difference between the screen and the window — measured, the status bar is a constant 62.8 CSS px while the toolbar moves between 58.1 and 98.1, so the top's share of the same quantity moves from 52 % to 39 % and no single fraction can replace the inset. Measured after, over the same 18 combinations: the photograph reaches **75.6 to 107 CSS px** above the window against 62.8 needed, and **83.5 to 138** below it against 57.2 to 98.1 needed | **CORRECTED — 18 SCROLL/TOOLBAR COMBINATIONS IN WEBKIT; DEVICE CONFIRMATION OUTSTANDING** |
| VF-46 | SP menu control, closed state, every width | Owner review on a physical iPhone: the mobile `menu` label is out of position under its mark and the control reads as broken | **The label was anchored to the wrong edge and set 27 % too small, and the two compounded.** Measured out of the approved `sp-reference.svg`: the word's ink runs x `331.43–357.57` (**26.14 px**) and the 3 × 3 mark's x `334–355` (21 px), so the reference centres both on **x 344.5** — one axis. The rule anchored the word with `left: 0` instead, which only coincides while the rendered word is exactly as wide as the approved one, and at `font-size: 8.6px` it was not: **21.16 px** in WebKit, putting its centre **2.5 px left** of the mark it sits under, and **25.00 px** in Chromium, which quantises small advances per glyph — one declaration, two positions, neither of them the reference. The size was independently wrong. In the shipping face — Jost, since the kit face is domain-locked and 412s on every origin including the deploy domains (Q-05) — the approved 26.14 px of ink needs **11 px with 0.66 px of tracking**, which also reproduces the reference x-height of 5.18 px (Jost's is 0.47 em → 5.17). The existing `top: 33px` is what identified 8.6 px as the defect: with `line-height: 7.6px` it puts the baseline at exactly the reference's 60.59 for an 11 px word. The method was validated against a control on the same frame — the drawer's `close` label at its shipped 12 px measures 28.4 px of ink against the reference's 29 — so the face is right and this one declaration was not. Centring is done with a translation rather than `text-align`, because the word's advance box is wider than the box it is centred in and all three engines clamp a centred line to the start edge once it overflows: measured, `text-align: center` left the word 1.15 px right of the mark in WebKit and 1.76 px in Chromium. `white-space: nowrap` is added at both breakpoints so no substituted face can wrap the word onto a second line — the shape the owner reported, and the one guard that holds whatever the text ends up measuring. `text-size-adjust: none` is belt-and-braces against iOS inflating it instead and is **not** relied on: Lightning CSS rewrites the authored `-webkit-` form to `-moz-` plus unprefixed for the configured targets, and whether the unprefixed property reaches iOS Safari cannot be established in this environment (the Linux WebKit build implements neither form and answers `CSS.supports()` false for both). Tailwind preflight's inherited `-webkit-text-size-adjust: 100%` on `html` is what actually disables inflation there. Measured after, ink centre against mark centre: **0.09 px in WebKit, 0.06 px in Chromium and Firefox**, ink width 26.12 / 27.33 / 26.14 against the reference 26.14, one line box on all three | CORRECTED — MEASURED ON THREE ENGINES AT 320/375/390/393/414/430/639; PENDING HUMAN REVIEW ON DEVICE |
| VF-47 | page top and page end, iOS 26 Safari, menu open | Owner review on a physical iPhone: opening and closing the menu brings a band back at the bottom of the screen | **Two independent defects, both of which make an edge candidate appear only while the drawer is open, which is why every desk check missed them.** `docs/ios26-tint-root-cause.md` §9 recorded the first as a limitation no page-level fix could reach; it was wrong. **(a) The top layer.** `showModal()` puts the drawer above every z-index on the page, `.chrome-shield`'s 120 included. At 390 px the panel is 250 px wide and full height, so it covers the midpoint of both window edges — the single point `fixedContainerEdges()` hit-tests — and being full-height and part-width it classifies as `IsSidebar`, a candidate, filling both bars with `--olive-aa`. Its `::backdrop` separately takes `containerResultFromBackdrop()`, which sets `isDimmingLayer` and with it the `preferExistingColor` branch that freezes the bars. The drawer is opened with `show()` instead: it takes its declared `z-index: 100`, generates no backdrop, and the shields keep the hit. Modality was already hand-rolled here — `inert` on the shell, an explicit focus trap — so the only thing genuinely lost is the `cancel` event, and Escape moved onto the dialog's own `keydown`. **(b) The scroll lock, which produced a paper band rather than an olive one and was never reported as a separate symptom.** `body.menu-locked` was `position: fixed`, making `<body>` a window-sized viewport-constrained box; the shield still took the hit and still failed as `TooSmall`, but the lineage walk then reached the body and accepted it, `primaryBackgroundColorForRenderer()` returning the paper `#ece7d8`. Measured against the source predicate at 320, 390 and 768: candidate `body.menu-locked` at both edges, with (a) already fixed. The lock moved to `[data-app-shell]`, which the shields are now siblings of, so their lineage is `body` → `html` and neither is viewport-constrained. The generalised finding — **any rule that makes an ancestor of a shield `position: fixed` or `sticky` defeats it** — is recorded in the root-cause document. Verified after on Chromium 151, Firefox 153 and WebKit 26.5 at 320, 390 and 768, menu open and closed: no candidate at either edge, `:modal` false, and the drawer's geometry, focus trap, Escape path, scroll lock and scroll restoration unchanged | CORRECTED — PREDICATE GREEN ON THREE ENGINES IN BOTH MENU STATES; DEVICE CONFIRMATION OUTSTANDING |
| VF-48 | page end, iOS 26 Safari, menu open | **Owner rejection of VF-47.** Opening and closing the menu still brings a band back at the bottom of the screen | **The scroll lock itself, for a third reason, and this one is not about the edge sampler at all.** VF-47 moved the lock from `<body>` to `[data-app-shell]` and cleared the candidate the sampler was finding; the predicate has been green in both menu states on three engines ever since, and the band stayed. What the lock also does is take the whole page out of flow, and by then the page had gained `.chapter-photo-mirror` — ordinary document content, the only kind of layer that reaches the strips the translucent bars are drawn over. A `position: fixed` subtree is clipped to the window, so the mirror stops reaching them. And removing the page from flow empties the root scroller, which makes the `scroll(root block)` timeline that holds the mirror still *inactive*: the animation stops producing a value and the mirror falls back to `transform: none`. Measured in WebKit at 390 x 844, scrolled to 2200, with the drawer open: `document.scrollingElement.scrollHeight` 6637 -> 844, the timeline's `currentTime` `37.98%` -> `null`, the mirror's transform `translateY(-608.39px)` -> `none`, and its box top -240 px -> +393 px on all four chapters. Either failure alone empties both strips. The lock is now an input-level one — non-passive `touchmove` and `wheel` cancelled while the drawer is open, `overscroll-behavior: contain` on the drawer, and the document scroll keys cancelled in the drawer's own `keydown` — so layout, the scroll offset, the scroll container and the scroll timeline are all left exactly as they were and nothing has to be restored on close. The generalised rule is recorded in globals.css where the lock used to be declared: **nothing may make an ancestor of `.chrome-shield` or `.chapter-photo-mirror` `position: fixed` or `sticky`, and nothing may take scrollable overflow away from the root scroller.** Verified on Chromium 151, Firefox 153 and WebKit 26.5 by `tests/e2e/production.spec.ts` — “the drawer's scroll lock leaves the mirror and the root scroller alone” and “the drawer holds the page still without taking it out of flow”, both red against the previous lock and green against this one | CORRECTED — GREEN ON THREE ENGINES; DEVICE CONFIRMATION OUTSTANDING |
| VF-49 | SP menu drawer, close button, open transition | Owner review on a physical iPhone: pressing the menu button draws a square frame around the close button, and the control reads as broken | **The page's own `:focus-visible` outline, drawn for a reader who never used a keyboard.** `dialog.show()` runs HTML's dialog focusing steps and lands on the close button, the drawer's first focusable descendant, and that focus carries no `FocusOptions`. WebKit then matches `:focus-visible` on it whatever the reader did — Codex traced the workaround that should prevent this, `EventHandler::setLatestFocusTrigger`, to a version that only inspected the innermost hit element, and this page's opener wraps its mark in nested `span`s, so the hit target is a span and the button is never recorded as click-focused (WebKit commit 311768 walks composed ancestors; the shipping build does not). The global rule then draws `2px solid currentColor` at `outline-offset: 4px` around the whole 29 x 39 button, label included. Measured at 390 x 844: `:focus-visible` true in WebKit 26.5, false in Chromium 151 and Firefox 153 — which is why no desk check ever saw it. The ring is aimed rather than removed: the modality of the opening click is read off `event.detail` (0 for Enter/Space, 1+ for a pointer) and restated with `focus({ focusVisible })` on the close button and again on the invoker when focus returns, so a keyboard reader keeps the indicator in both directions and WCAG 2.4.7 / 2.4.11 are untouched. `focus()` on an already-focused element is a no-op, so the call follows a `blur()`. Verified on all three engines by `tests/e2e/production.spec.ts` — “the drawer's focus ring answers the modality that opened it”, red before the fix | CORRECTED — GREEN ON THREE ENGINES; DEVICE CONFIRMATION OUTSTANDING |
