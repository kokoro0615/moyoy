...
| M-06 browser-bar mirror (DA-MEDIA-01) | `.chapter-photo-mirror` inside each `.chapter-photo`, via `--chapter-mirror-from/to` | root scroll timeline, CSS only | the whole document; endpoints derived from the chapter's measured document top and the document's maximum scroll | counter-translation of exactly `scrollY − 240 − chapterDocumentTop`, so the layer is screen-stationary | no duration; the scroll is the timeline | endpoints re-derived on load, resize, `visualViewport` resize, orientation, `pageshow`, and whenever `innerHeight` changes — never during a scroll | identical model at every width; the travel constraint in `docs/scroll-motion-spec.md` §4.2 applies only below 640 px | none: the plate covers it everywhere inside the window | one composited layer per chapter, sharing the plate's decoded image | `display: none` — the plate leaves the window and the whole frame shows in flow | assert `position: absolute`, a constant box top of −240 across scroll offsets, a constant document height, and one accessible image per chapter | measured on device at 0.05–0.64 CSS px on the bottom edge; **top edge `UNVERIFIED`** |
...
| Chapter photograph travel, compact composition | reduced from the window's hidden remainder to the screen's — 53.7 px instead of 213.7 on the measured device | the approved SP derivative is 927.7 CSS px tall against an 874 px screen and cannot both traverse its remainder and reach past both window edges; DA-MEDIA-01 asks for a photograph that does not scroll, so travel is what gives (Q-23b) |
| Chapter photograph crop origin, compact composition | starts about a status bar above the window rather than level with it | a photograph whose top edge is at the window's top edge cannot appear above that edge, and appearing above it is the whole requirement (Q-23b) |
| Chapter photograph, DOM | present twice per chapter — the viewport-fixed plate, and a document-level mirror | iOS Safari's bars are translucent over the document's own scrolled paint and a fixed box is clipped to the window, so no single layer can be both pinned and present in the strips |
